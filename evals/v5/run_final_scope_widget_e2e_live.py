"""CLI for FINAL scope/widget E2E live runtime eval."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Validate frozen suite, authority env, and artifact guards only",
    )
    parser.add_argument(
        "--offline",
        action="store_true",
    ...
    parser.add_argument(
        "--live",
        action="store_true",
        help="Run one owner-approved FINAL scope/widget live attempt",
    )
    parser.add_argument(
    ...
    if args.offline:
        print(
            "Use pytest tests/test_final_scope_widget_e2e_live_harness.py for offline harness",
            file=sys.stderr,
        )
        return 0
