...
"""Implantation golden eval (PRODUCT_WORK_PLAN stage 0)."""
import argparse
import os
import sys
...
def main(argv: list[str] | None = None) -> int:
    argv = list(argv or [])
    ap = argparse.ArgumentParser(description="Implantation golden eval (demo client)", allow_abbrev=False)
    ...
    path = (os.getenv("IMPLANT_EVAL_PATH") or "").strip() or here("implant_golden.json")
    ...
    env_csv = (os.getenv("IMPLANT_EVAL_CASE_ID") or "").strip()
    if env_csv:
    ...
    elif (os.getenv("IMPLANT_EVAL_CLIENT") or os.getenv("E2E_SMOKE_CLIENT") or "").strip():
        client_filter = (
            os.getenv("IMPLANT_EVAL_CLIENT") or os.getenv("E2E_SMOKE_CLIENT") or ""
        ).strip().lower()

    print(f"\n=== implant golden eval ({path}) ===\n", flush=True)
