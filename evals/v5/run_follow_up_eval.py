...
"""Follow-up golden eval (PRODUCT_WORK_PLAN stage 4a)."""
import argparse
import os
import sys
...
def main(argv: list[str] | None = None) -> int:
    argv = list(argv or [])
    ap = argparse.ArgumentParser(description="Follow-up golden eval (demo client)", allow_abbrev=False)
    ...
    path = (os.getenv("FOLLOW_UP_EVAL_PATH") or "").strip() or here("follow_up_golden.json")
    ...
    env_csv = (os.getenv("FOLLOW_UP_EVAL_CASE_ID") or "").strip()
    if env_csv:
    ...
    elif (os.getenv("FOLLOW_UP_EVAL_CLIENT") or os.getenv("E2E_SMOKE_CLIENT") or "").strip():
        client_filter = (
            os.getenv("FOLLOW_UP_EVAL_CLIENT") or os.getenv("E2E_SMOKE_CLIENT") or ""
        ).strip().lower()

    print(f"\n=== follow-up golden eval ({path}) ===\n", flush=True)
