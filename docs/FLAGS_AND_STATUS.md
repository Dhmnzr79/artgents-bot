...
|---|---|---|
| `BRAND_FILTER_ON` | Бренд-фильтр + бюджетный якорь на ценовом пути имплантов: «корейские/нобель» → один бренд; «подешевле» → честный якорь (доступный + рекомендованный + платёж + консультация); прочее «дешевле» → тёплый fallback | Построен и проверен вживую (2026-07-10), деньги пришпилены, `numeric_fact_gate: pass`. Перед флипом: (1) прогнать product-сьюту — у фичи **новый ранний хук** в `ask_turn`; (2) опц. усилить Impro-якорь в брифе («рекомендуем/пожизненная гарантия», сейчас «чуть дороже») |
...
| «какие подешевле», «корейские», «нобель» | `BRAND_FILTER_ON=1` | бренд-путь не срабатывает (`brand_money_path=None`) |
...
- **Бренд-фильтр + бюджетный якорь** (`BRAND_FILTER_ON`, OFF): закрыт долг S6; якорь **портируемый** (доступный = min-цена, рекомендованный = `recommended:true`, услуга-якорь из `features.yaml` — без брендовых литералов в коде).

**Осталось:**
1. Флип `BRAND_FILTER_ON` (после product-сьюты) + опц. усиление Impro-якоря.
2. **Этап 7 — концерн-схемы** (спек сверен с кодом, см. `FULLCONTEXT_ROADMAP.md`). Первый шаг: снести мёртвые `limits:` + перетегировать `free_implant_consult` `promo→benefit`.
3. Интент «Доверие» («врачи опытные?», «отзывы?») — небольшой.
