...
### A. Ждёт твоего продуктового решения (технически работает)
...
| `PRICE_STRICT_SERVICE_ON` | Цена без явно названной услуги → честный defer, не выдуманная цена | Ветка out-of-base. Проверено (H2/H3). Кандидат на дефолт-ON |
| `PRICE_SYMPTOM_CONSULT_ON` | «Цена по симптому» → тёплое приглашение на консультацию | Медзона-политика. Кандидат на дефолт-ON |
| `BOOKING_DATE_DEFER_ON` | Не подтверждать/не эхоить дату и слот записи | Сделано 2026-07-09. Кандидат на дефолт-ON |
...
|---|---|---|
| H2, H3 (цена вне базы) | `PRICE_STRICT_SERVICE_ON=1` | бот выдаст fuzzy-цену |
| G1 «шатается зуб, сколько» | `PRICE_SYMPTOM_CONSULT_ON=1` | сработает strict-defer (другой путь) |
