"""Frozen contract for S66 default FullContext authority live verification."""
...
MEASUREMENT_ID = "s66_default_authority_live"
SUITE_ID = "s66_default_authority_live"
CLIENT_ID = "demo"
LIVE_QUESTION = "Что такое All-on-4?"
LIVE_ENDPOINT = "/ask"
MAX_HTTP_TURNS = 1
MAX_PROVIDER_CALLS = 5
MAX_INGRESS_CALLS = 1
MAX_PLANNER_CALLS = 1
MAX_BOUNDARY_CALLS = 1
MAX_COMPOSER_CALLS = 1
...
LIVE_RAW_ARTIFACT_PATH = LIVE_ARTIFACTS_DIR / "s66_default_authority_live_raw.json"
LIVE_RESULT_ARTIFACT_PATH = LIVE_ARTIFACTS_DIR / "s66_default_authority_live_result.json"
LIVE_MANIFEST_ARTIFACT_PATH = LIVE_ARTIFACTS_DIR / "s66_default_authority_live_manifest.json"
LIVE_MANUAL_REVIEW_ARTIFACT_PATH = (
    LIVE_ARTIFACTS_DIR / "s66_default_authority_live_manual_review.json"
)
LIVE_ATTEMPT_MARKER_PATH = LIVE_ARTIFACTS_DIR / "s66_default_authority_live_attempt.json"
LIVE_CALL_LEDGER_PATH = LIVE_ARTIFACTS_DIR / "s66_default_authority_live_call_ledger.jsonl"
LIVE_AUDIT_LOG_PATH = LIVE_ARTIFACTS_DIR / "s66_default_authority_live_audit.log"
...
AuthoritySource = Literal["config_default"]
...
class DefaultAuthorityEnvError(HarnessConfigError):
    """TARGET_FULLCONTEXT_DEV must be absent for S66."""
...
def assert_target_fullcontext_env_absent() -> None:
    if "TARGET_FULLCONTEXT_DEV" in os.environ:
        raise DefaultAuthorityEnvError(
            "TARGET_FULLCONTEXT_DEV must be absent for S66; "
            f"found={os.environ.get('TARGET_FULLCONTEXT_DEV')!r}"
        )


def resolve_default_authority_proof() -> dict[str, Any]:
    """Fail-closed proof that authority comes from config default, not env."""
    ...
    if not config.TARGET_FULLCONTEXT_DEV:
        raise HarnessConfigError(
            "config.TARGET_FULLCONTEXT_DEV must resolve True without env override"
        )
    return {
        "env_present": False,
    ...
    return prepare_json_artifact_payload(
        {
            ...
            "turn": {
                "endpoint": turn_result.get("endpoint"),
                "request": turn_result.get("request"),
                "answer_text": turn_result.get("answer_text"),
                "automated_turn_verdict": turn_result.get("automated_turn_verdict"),
