...
"""Planner-lite golden eval (PRODUCT_WORK_PLAN stage 4b)."""
import argparse
import os
import sys
...
def main(argv: list[str] | None = None) -> int:
    argv = list(argv or [])
    ap = argparse.ArgumentParser(description="Planner-lite golden eval (demo client)", allow_abbrev=False)
    ...
    path = (os.getenv("PLANNER_EVAL_PATH") or "").strip() or here("planner_golden.json")
    ...
    env_csv = (os.getenv("PLANNER_EVAL_CASE_ID") or "").strip()
    if env_csv:
    ...
    elif (os.getenv("PLANNER_EVAL_CLIENT") or os.getenv("E2E_SMOKE_CLIENT") or "").strip():
        client_filter = (
            os.getenv("PLANNER_EVAL_CLIENT") or os.getenv("E2E_SMOKE_CLIENT") or ""
        ).strip().lower()

    print(f"\n=== planner-lite golden eval ({path}) ===\n", flush=True)
