...
> **Фазы 0–3 завершены.** Продолжение работ — в **`FULLCONTEXT_ROADMAP.md`** (инварианты §2 и правила тестов §5 этого документа остаются нормативными).

**Статус:** план работ (2026-07). Реализация — в Cursor, поэтапно.
**Прогресс:** Фаза 0 ✓ · Фаза 1 ✓ · Фаза 2 ✓ · Фаза 3 ✓
...
**Готово:** `answer_packet` в телеметрии корректен; ни одна запрещённая промо-карточка не попадает в пакет (проверяется тестом на структуру). **Доработка:** строгое подавление промо на составных вопросах с чувствительным аспектом (`pain` и др. из `blocked_aspects_for_promo`) — даже при `price` в плане; тест `test_promo_suppressed_on_composite_price_pain`.
