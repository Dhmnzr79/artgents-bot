...
require_trusted_release_directory() {
  ...
  if [ -L "$path" ] || [ ! -d "$path" ]; then fail "release directory must be an ordinary directory: $path"; fi
  owner=$(stat -c '%u:%g' "$path")
  perms=$(stat -c '%a' "$path")
  if [ "$owner" != "0:0" ]; then fail "release directory must be root-owned: $path"; fi
  if [ "$((8#${perms} & 022))" -ne 0 ]; then fail "release directory must not be group/world writable: $path"; fi
}

ensure_release_root() {
  require_trusted_release_directory "$CURRENT_ROOT"
  if [ -L "$RELEASES_ROOT" ]; then fail "release root must not be a symlink"; fi
  if [ ! -e "$RELEASES_ROOT" ]; then install -d -o root -g root -m 0755 "$RELEASES_ROOT"; fi
...
try:
    for path in root.rglob("*"):
        ...
        if stat.S_ISLNK(mode): sys.exit(1)
        if stat.S_ISDIR(mode): actual_dirs.add(rel)
        elif stat.S_ISREG(mode): actual_files.add(rel)
        else: sys.exit(1)
except OSError:
    sys.exit(1)
if actual_dirs != expected_dirs or actual_files != expected_files: sys.exit(1)
for rel in expected_files:
    size = (root / rel).stat().st_size
    if size < 1 or size > 1024 * 1024: sys.exit(1)
PY
}
...
prepare_release_assets_from_image() (
  ...
  local revision="" destination="${RELEASES_ROOT}/${source_sha}" staging="" cid="" state=""
  ...
  if [ "$revision" != "$source_sha" ]; then fail "image revision label does not match source_sha"; fi
  staging=$(mktemp -d "${RELEASES_ROOT}/.release-${source_sha}.XXXXXX")
  install -d -o root -g root -m 0755 "${staging}/deploy/production"
  if ! cid=$(docker create "$image_ref"); then fail "docker create failed for release asset extraction"; fi
  if ! state=$(docker inspect --format '{{.State.Status}}' "$cid"); then fail "failed to inspect release extraction container"; fi
  if [ "$state" != "created" ]; then fail "release assets must be extracted from a stopped created container"; fi
  ...
  if ! validate_release_tree "$staging"; then fail "release extraction contains invalid or unexpected assets"; fi
  ...
  if ! validate_release_tree_metadata "$staging"; then fail "release extraction has unsafe ownership or permissions"; fi
  if [ -e "$destination" ] || [ -L "$destination" ]; then
    require_trusted_release_directory "$destination"
    if ! validate_release_tree "$destination"; then fail "existing release tree is invalid"; fi
    if ! validate_release_tree_metadata "$destination"; then fail "existing release tree has unsafe ownership or permissions"; fi
    ...
    if ! mv -- "$staging" "$destination"; then fail "failed to install source-specific release tree"; fi
    staging=""
  fi
...
validate_receipt_path_metadata() {
  ...
  perms=$(stat -c '%a' "$path")
  if [ "$((8#${perms} & 022))" -ne 0 ]; then fail "receipt must not be group/world writable"; fi
}
...
validate_backup_receipt_path() {
  ...
  if [ "$((8#${perms} & 022))" -ne 0 ]; then fail "backup receipt must not be group/world writable"; fi
  printf '%s' "$canonical"
}
