...
Открытые вопросы владельцу (если есть): <то, что требует решения Клода/Дениса>
```

При **любом** нарушении блока A → сразу ❌, без разбора остального.
Если Ревьюер **сам не уверен** (архитектурная развилка, спорный инвариант) → вердикт `❓ эскалация` и вопрос выносится Клоду на контрольной точке.
