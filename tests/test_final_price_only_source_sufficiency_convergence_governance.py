"""PRE-CODE checker for FINAL_PRICE_ONLY_SOURCE_SUFFICIENCY_CONVERGENCE governance (Phase 1 only)."""
...
SEAM_AUDIT_PATH = (
    ...
    / "evidence"
    / "runtime"
    / "FINAL_PRICE_ONLY_SOURCE_SUFFICIENCY_CONVERGENCE_SEAM_AUDIT.md"
)
TASK_PATH = _REPO_ROOT / "TASK.md"
GOVERNANCE_BASELINE_HEAD = "c4de72c"
MILESTONE = "FINAL_PRICE_ONLY_SOURCE_SUFFICIENCY_CONVERGENCE"


def test_seam_audit_exists_and_covers_price_only_source_sufficiency() -> None:
...
def test_task_governance_section_and_acceptance_matrix() -> None:
    ...
    assert "test_final_price_only_source_sufficiency_convergence_governance.py" in text
    assert "is_price_only_offer_source_sufficient" in section
    assert "composer_request_source_mismatch" in section
    assert "price_only_source_sufficiency" in section
...
def test_implementation_allowlist_present() -> None:
    ...
    for path in (
        "contracts/price_only_source_sufficiency.py",
        "core/target_composer_request.py",
        "core/target_scoped_response_evidence.py",
        "tests/test_final_price_only_source_sufficiency_convergence_implementation.py",
    ):
