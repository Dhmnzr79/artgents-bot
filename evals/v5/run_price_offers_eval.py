...
"""Price offers golden eval (PRODUCT_WORK_PLAN stage 3)."""
import argparse
import os
import sys
...
def main(argv: list[str] | None = None) -> int:
    argv = list(argv or [])
    ap = argparse.ArgumentParser(description="Price offers golden eval (demo client)", allow_abbrev=False)
    ...
    path = (os.getenv("PRICE_OFFERS_EVAL_PATH") or "").strip() or here("price_offers_golden.json")
    ...
    env_csv = (os.getenv("PRICE_OFFERS_EVAL_CASE_ID") or "").strip()
    if env_csv:
    ...
    elif (os.getenv("PRICE_OFFERS_EVAL_CLIENT") or os.getenv("E2E_SMOKE_CLIENT") or "").strip():
        client_filter = (
            os.getenv("PRICE_OFFERS_EVAL_CLIENT") or os.getenv("E2E_SMOKE_CLIENT") or ""
        ).strip().lower()

    print(f"\n=== price offers golden eval ({path}) ===\n", flush=True)
