...
def test_real_demo_pipeline_crosses_s33_s34_s39_once_and_preserves_sidecars() -> None:
...
def test_real_s33_failure_reaches_neither_s34_nor_s39_backends() -> None:
    inputs = _real_inputs()
    inputs["policy_request"] = object()
    composer = RecordingComposerBackend()
    semantic = RecordingSemanticBackend()
    with pytest.raises(TargetResponsePolicyBuildError) as caught:
        run_target_offline_policy_bound_verified_response_pipeline(
            **inputs,  # type: ignore[arg-type]
            composer_backend=composer,
            semantic_backend=semantic,
        )
    assert caught.value.code == "response_policy_request_invalid"
...
def test_real_s39_s36_failure_reaches_neither_backend_and_propagates_exact_error() -> None:
    inputs = _real_inputs()
    inputs["user_message"] = "  "
    composer = RecordingComposerBackend()
    semantic = RecordingSemanticBackend()
    ...
    assert caught.value.code == "composer_request_message_invalid"
    assert composer.invocations == []
    assert semantic.invocations == []


def test_real_s39_s37_failure_calls_once_and_never_reaches_semantic_backend() -> None:
...
def test_real_s39_s38_numeric_rejection_occurs_before_semantic_backend() -> None:
    composer = RecordingComposerBackend(
