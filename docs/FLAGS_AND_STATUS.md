...
|---|---|---|
| `PRICE_STRICT_SERVICE_ON` | Цена без явно названной услуги → честный defer | Баг фокуса: учитывает только сервисный фокус, не ситуационный → на «нет одного зуба… а сколько?» деферит, теряя цену за зуб (ps03). Флипать в дефолт только после фикса фокуса. Пока opt-in |
