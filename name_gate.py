...
_LEAD_REFUSAL_PHRASE_RX = re.compile(
    r"^я\s+(?:не\s+)?(?:передумал(?:а)?|буду|хочу)\b",
    re.I | re.U,
)
...
_LEAD_NAME_REJECT = frozenset(
    {
        "боюсь", "хочу", "буду", "передумал", "передумала",
        "переживаю", "переживал", "переживала", "беспокоюсь",
...
def hard_reject_lead_name(text: str) -> bool:
    ...
    if _LEAD_REFUSAL_PHRASE_RX.match(s):
        return True
