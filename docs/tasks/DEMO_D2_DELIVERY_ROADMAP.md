...
CP5-B05 draft связал typed brand ID с активными ценами этого бренда,
...
Targeted offline: 85 passed; assembled CP5 scenario/dialogue: 105 passed;
независимая проверка назначена в другом чате.
Это пока не доказательство распознавания бренда настоящей моделью и не HTTP
cutover. Для B07/A06/A09/повторённого
неизвестного термина новые отдельные runtime-ветки не планируются: при
неясности — одно короткое уточнение, при отсутствии личной цены или данных —
честный пробел и разрешённая консультация, при A09 — опубликованная единица
без умножения. Уже реализованные части этих случаев не переписываются.
