"""CLI for FINAL scope/widget E2E retry4 live runtime eval."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Validate frozen suite, authority env, post-S69 seams, and retry4 artifact guards only",
    )
    parser.add_argument(
        "--offline",
        action="store_true",
    ...
    parser.add_argument(
        "--live",
        action="store_true",
        help="Run one owner-approved FINAL scope/widget retry4 live attempt",
    )
    parser.add_argument(
        "--owner-override-attempt-marker",
        action="store_true",
        help="Allow live prep when retry4 attempt marker exists (owner approval only)",
    )
    ...
    if args.offline:
        print(
            "Use pytest tests/test_final_scope_widget_e2e_retry4_live_harness.py for offline harness",
            file=sys.stderr,
        )
        return 0
