...
"""Answer slots golden eval (PRODUCT_WORK_PLAN stage 2)."""
import argparse
import os
import sys
...
def main(argv: list[str] | None = None) -> int:
    argv = list(argv or [])
    ap = argparse.ArgumentParser(description="Answer slots golden eval (demo client)", allow_abbrev=False)
    ...
    path = (os.getenv("ANSWER_SLOTS_EVAL_PATH") or "").strip() or here("answer_slots_golden.json")
    ...
    env_csv = (os.getenv("ANSWER_SLOTS_EVAL_CASE_ID") or "").strip()
    if env_csv:
    ...
    elif (os.getenv("ANSWER_SLOTS_EVAL_CLIENT") or os.getenv("E2E_SMOKE_CLIENT") or "").strip():
        client_filter = (
            os.getenv("ANSWER_SLOTS_EVAL_CLIENT") or os.getenv("E2E_SMOKE_CLIENT") or ""
        ).strip().lower()

    print(f"\n=== answer slots golden eval ({path}) ===\n", flush=True)
